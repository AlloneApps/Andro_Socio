...
public class SourceLocationActivity extends AppCompatActivity implements OnMapReadyCallback,
        GoogleApiClient.ConnectionCallbacks,
        GoogleApiClient.OnConnectionFailedListener,
        com.google.android.gms.location.LocationListener {

    private final static int PLAY_SERVICES_RESOLUTION_REQUEST = 999;
    private static String TAG = SourceLocationActivity.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bundle = getIntent().getBundleExtra("LOC");

        if (bundle != null) {
            alreadySelectedLocation = new Location("");
            alreadySelectedLocation.setLatitude(bundle.getDouble(AppConstants.LATITUDE, 0.0));
            alreadySelectedLocation.setLongitude(bundle.getDouble(AppConstants.LONGITUDE, 0.0));

            Log.d(TAG, "onPlaceSelected: before call: " + bundle.getDouble(AppConstants.LATITUDE, 0.0) + "," + bundle.getDouble(AppConstants.LONGITUDE, 0.0));
            Log.d(TAG, "onCreate: selected Loc: " + alreadySelectedLocation.toString());
        } else {
        ...
        if (checkPlayServices()) {
            ...
            if (Utils.checkLocationPermission(SourceLocationActivity.this)) {
    ...
    public void onResume() {
        super.onResume();
        Objects.requireNonNull(SourceLocationActivity.this.getSupportActionBar()).hide();
    }
    ...
    public void onDestroy() {
        super.onDestroy();
        Objects.requireNonNull(SourceLocationActivity.this.getSupportActionBar()).show();
    }
...
//            map.clear();
        ...
        if (isSourceSet) {
            if (map != null) {
                map.addMarker(new MarkerOptions()
                        ...
                        .icon(Utils.getBitmapDescriptor(SourceLocationActivity.this, R.drawable.ic_pin_drop_pickup)));
            }
        }
    ...
    private void requestPermission() {
        if (ActivityCompat.shouldShowRequestPermissionRationale(SourceLocationActivity.this, Manifest.permission.ACCESS_FINE_LOCATION)
                || ActivityCompat.shouldShowRequestPermissionRationale(SourceLocationActivity.this, Manifest.permission.ACCESS_COARSE_LOCATION)) {
        ...
        try {
            if (requestCode == AppConstants.PERMISSION_REQUEST_LOCATION) {
                ...
                if (grantResults.length == 2 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    if (Utils.isLocationEnabled(SourceLocationActivity.this)) {
        ...
        result.setResultCallback(locationSettingsResult -> {
            ...
            switch (status.getStatusCode()) {
                ...
                case LocationSettingsStatusCodes.RESOLUTION_REQUIRED:
                    try {
                        status.startResolutionForResult(SourceLocationActivity.this, LOCATION_SETTINGS_CODE);
